...
func (c *Purge) Apply(a *Applier) (any, error) {
	...
	err := a.Site(func(tx *sql.Tx) error {
		...
		// Each group's own file, through its own log.
		groups, err := groupIDs(tx, "")
		if err != nil {
			return err
		}
		for _, g := range groups {
			if err := send(tx, &PurgeGroup{GroupID: g, Before: c.Before}, c.Before); err != nil {
				return err
			}
		}
		return nil
...
// PurgeGroup is Purge's part in one group's file. Purge sends one to each
// group's log.
type PurgeGroup struct {
	GroupID int64
	Before  int64
}

func (c *PurgeGroup) Apply(a *Applier) (any, error) {
	id := c.GroupID
	return nil, a.Group(id, func(tx *sql.Tx) error {
		for _, q := range []string{
